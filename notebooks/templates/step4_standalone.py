...
print("=" * 80)
print("## 🛑 最高协议：先确认逻辑，严禁直接出代码 (CRITICAL)")
print("=" * 80)
print("""
⚠️ **ZERO CODE POLICY (第一轮强制约束)**：

在看到我的分析需求后，你**绝对禁止**在第一轮回复中生成 `update_dashboard` 代码。
你必须先执行以下 **[确认逻辑]** 步骤：

1. **复述需求**：简述你理解的分析目的。
2. **定义口径**：准确列出核心指标的数学公式（如：损益 = A - B）。
3. **核对字段**：列出实现该逻辑所需的 DataFrame 列名。
4. **询问确认**：停止输出，等待我说“确认/OK”后，你再在下一轮生成代码。

**提示**：如果你违反此协议直接生成代码，我会视作严重违规。
""")
...
📌 **第四步：分析自我叙述 (Self-documenting)**

⚠️ **重要：为了生成高质量的分析报告，你必须在 `update_dashboard` 函数的 Docstring 中包含分析摘要。**
...
def update_dashboard(*args):
    ...
    [REPORT_METADATA]
    ### 1. 业务背景
    详细阐述：**分析目的**（想通过数据解决什么问题）以及**为什么要分析**（该指标对业务的价值、背景意义）。
    
    ### 2. 分析逻辑
    详细解释：**具体的分析逻辑**（例如：如何定义逾期、保费是如何加权的、过滤了哪些特殊保单、聚合的口径是什么）。
    
    ### 3. 核心维度
    列出本次分析使用的动态聚合轴（或其他关键分析维度）。
    [/REPORT_METADATA]
    \"\"\"
    # 代码逻辑...
```
...
**为什么？**
生成报告时，系统会自动提取此段文字作为报告的“分析概述”章节。
""")
...
def update_dashboard(*args):
    ...
    [REPORT_METADATA]
    ### 1. 业务背景
    [阐述分析目的：想解决什么问题？为什么要进行这项分析？背后的业务驱动力是什么？]
    
    ### 2. 分析逻辑
    [详细解释计算逻辑：包含数据口径、计算公式、特殊的过滤说明以及为何选择这些指标。]
    [/REPORT_METADATA]
    \"\"\"
    # 1. 物理隔离获取业务过滤值
